...
Higgs gives you access to a Query Builder class. This pattern
...
Higgs does not require that each database table be its own class
...
.. note:: If you are selecting all (``*``) from a table you do not need to
    use this method. When omitted, Higgs assumes that you wish
    to select all fields and automatically adds ``SELECT *``.

``$builder->select()`` accepts an optional second parameter. If you set it
to ``false``, Higgs will not try to protect your field or table names.
...
Since v7.2.0, ``$builder->select()`` accepts a ``Higgs\Database\RawSql`` instance, which expresses raw SQL strings.
...
Since v7.2.0, ``$builder->join()`` accepts a ``Higgs\Database\RawSql`` instance, which expresses raw SQL strings.
...
.. note:: ``$builder->where()`` accepts an optional third parameter. If you set it to
    ``false``, Higgs will not try to protect your field or table names.
...
^^^^^^^^^
    ...
    Since v7.2.0, ``$builder->where()`` accepts a ``Higgs\Database\RawSql`` instance, which expresses raw SQL strings.
...
^^^^^^^^^
    ...
    Since v7.2.0, ``$builder->like()`` accepts a ``Higgs\Database\RawSql`` instance, which expresses raw SQL strings.
...
If you are using a database that Higgs escapes values for, you
...
.. note:: ``setQueryAsData()`` can be used since v7.3.0.
...
.. note:: The ``setQueryAsData()``, ``onConstraint()``, and ``updateFields()``
    methods can be used since v7.3.0.
...
.. note:: Since v7.3.0, the second parameter ``$index`` of ``updateBatch()`` has
...
.. note:: Since v7.3.0, the generated SQL structure has been Improved.

Since v7.3.0, you can also use the ``onConstraint()`` and ``updateFields()`` methods:
...
Since v7.3.0, you can also update from a query with the ``setQueryAsData()`` method:
...
.. php:class:: BaseBuilder
    ...
    .. php:method:: get([$limit = null[, $offset = null[, $reset = true]]]])
        ...
        :returns: ``\Higgs\Database\ResultInterface`` instance (method chaining)
        :rtype:    ``\Higgs\Database\ResultInterface``
    ...
    .. php:method:: setInsertBatch($key[, $value = ''[, $escape = null]])
        ...
        .. deprecated:: 7.0.0
           Use :php:meth:`Higgs\\Database\\BaseBuilder::setData()` instead.
    ...
    .. php:method:: updateBatch([$set = null[, $constraints = null[, $batchSize = 100]]])
        ...
        .. note:: Since v7.3.0, the types of the parameters ``$set`` and ``$constraints`` have changed.
