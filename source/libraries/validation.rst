...
Higgs provides a comprehensive data validation class that
...
Before explaining Higgs's approach to data validation, let's
...
What follows is a "hands on" tutorial for implementing Higgs's Form
Validation.
...
.. note:: The :ref:`$this->validator->getValidated() <validation-getting-validated-data>`
    method can be used since v7.4.0.
...
Higgs has two kinds of Validation rule classes.
The traditional rule classes (**Traditional Rules**) have the namespace ``Higgs\Validation``,
and the new classes (**Strict Rules**) have ``Higgs\Validation\StrictRules``, which provide strict validation.

.. note:: Since v7.3.0, **Strict Rules** are used by default for better security.
...
Higgs lets you set as many validation rules as you need for a
...
.. warning:: Prior to v7.2.0, this method's third parameter, ``$rules``, was typehinted to accept
    ``string``. In v7.2.0 and after, the typehint was removed to allow arrays, too. To avoid LSP being
...
.. note:: Prior to v7.4.4, due to a bug, the wildcard ``*`` validated data in incorrect
...
.. note:: Prior to v7.4.0, this method's second parameter, ``$rule``, was
    typehinted to accept ``string``. In v7.4.0 and after, the typehint was
